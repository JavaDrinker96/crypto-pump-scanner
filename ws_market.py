...
class BybitMarketWS:
    def __init__(self, testnet=False, max_candles=180):
        ...
        self.max_candles=max_candles; self._lock=threading.RLock(); self._ws=None; self._thread=None; self._stop=False; self._topics=set(); self._ready=threading.Event(); self._sub_id=0
    ...
    def _run(self):
        while not self._stop:
            try:
                self._ws=websocket.WebSocketApp(self.url,on_open=self._on_open,on_message=self._on_message,on_error=self._on_error,on_close=self._on_close); self._ws.run_forever(ping_interval=20,ping_timeout=10)
    ...
    def ensure_symbols(self,symbols):
        ...
        for s in symbols:
            z=self.sym(s)
            for t in (f'tickers.{z}',f'publicTrade.{z}',f'orderbook.50.{z}',f'kline.1.{z}'):
                if t not in self._topics: new.append(t)
        if new:
            self._topics.update(new)
            if self._ws: self._send(new)
