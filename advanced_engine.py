...
@dataclass
class Signal:
 symbol:str; side:str; price:float; atr:float; rsi:float; vol:float; flow:float; book:float; vwap:float; move5:float; score:float; reason:str
@dataclass
class Position:
...
class Engine:
 ...
 def book(self,s,p):
  try:
   o=self.c.fetch_order_book(s,limit=50);b=sum(q*p for p,q in o.get('bids',[]) if p>=p*(1-self.depth/100));a=sum(q*p for p,q in o.get('asks',[]) if p<=p*(1+self.depth/100));bid=o.get('bids',[[p,0]])[0][0];ask=o.get('asks',[[p,0]])[0][0];return b/max(a+b,1e-12),(ask-bid)/p*100
 ...
 def signal(self,s):
  ...
  side='long' if long else 'short';q=flow if long else 1-flow;bi=book if long else 1-book;score=min(.3*min(vr/5,1)+.2*min(abs(m5)/.05,1)+.25*q+.15*max((bi-.5)*2,0)+.1,1);return Signal(s,side,p,a,r,vr,flow,book,vd,m5*100,score,'continuation' if long else 'exhaustion')
 def equity(self):
 ...
 def run(self,symbols):
  for s in symbols:
   try:
    sig=self.signal(s)
    if sig and self.alert:self.alert(f"🚨 {sig.side.upper()} {s} score={sig.score:.2f} RSI={sig.rsi:.1f} vol={sig.vol:.1f}x flow={sig.flow:.2f} book={sig.book:.2f}")
